        ...
        if self.wait:
            ...
            if (self.rect.x - WALL_SIZE) % PLAYER_SIZE == 0 \
                ...
                hitlist = pygame.sprite.spritecollide(self,
                        level.wall_list, False)
                if not hitlist:
                    choices.append([self.direction[0],
                                   self.direction[1]] * 2)
                self.rect.x -= self.direction[0]
                self.rect.y -= self.direction[1]
...
class Player(pygame.sprite.Sprite):
    ...
    def move(self, level):
        ...
        hitlist = pygame.sprite.spritecollide(self, level.wall_list,
                False)
        if hitlist:
            self.rect.x -= self.queue[0]
            self.rect.y -= self.queue[1]
            self.rect.x += self.direction[0]
            self.rect.y += self.direction[1]
            seclist = pygame.sprite.spritecollide(self,
                    level.wall_list, False)
            if seclist:
                self.rect.x -= self.direction[0]
                self.rect.y -= self.direction[1]
...
class Text:
    ...
    def update(self, screen, text):
        cover = pygame.Surface([len(text) * self.size // 2.1, self.size
                               * 0.7])
        cover.fill([0, 0, 30])
...
class Map:
    ...
    def __init__(self):
        ...
        enemies = [Enemy(WALL_SIZE + 8 * PLAYER_SIZE, WALL_SIZE + 6
                  * PLAYER_SIZE, [255, 0, 0]), Enemy(WALL_SIZE + 8
                  * PLAYER_SIZE, WALL_SIZE + 6 * PLAYER_SIZE, [0, 255,
                  0]), Enemy(WALL_SIZE + 8 * PLAYER_SIZE, WALL_SIZE + 6
                  * PLAYER_SIZE, [0, 255, 255]), Enemy(WALL_SIZE + 8
                  * PLAYER_SIZE, WALL_SIZE + 6 * PLAYER_SIZE, [125, 0,
                  125])]
        self.enemy_list = pygame.sprite.Group()
        for i in enemies:
            self.enemy_list.add(i)
        food = [[WALL_SIZE + PLAYER_SIZE / 2 - 2.5 + PLAYER_SIZE * (i
                % 17), WALL_SIZE + PLAYER_SIZE / 2 - 2.5 + i // 17
